...
class Trader:
    ...
    def _narrow_spread_taking(
        ...
        """
        Taking opportunistico nel regime di narrow spread, derivato dall'HMM.

        Logica economica:
          Quando lo spread si stringe (≤6-7 tick) il mercato è in un regime
          di momentum locale dove imbalance_tot predice la direzione con
          correlazione 0.87 e accuracy 96-98%.

          spread≤6 + imbtot>0: prezzo sta salendo (momentum bullish)
            → compra al best_ask, EV +1.38 tick (acc 98.3%)
          spread≤7 + imbtot<0: prezzo sta scendendo (momentum bearish)
            → vendi al best_bid, EV +0.42 tick (acc 96.4%)

        Usa solo quando entrambe le condizioni sono soddisfatte.
        Size conservativa (NARROW_TAKE_SIZE=10) per non saturare il limite.
        """
        orders: List[Order] = []
        ...
        # Calcola imbalance_tot (L1 + L2) — stesso identico calcolo del fair
        bv1 =  od.buy_orders.get(bb, 0)
        av1 = -od.sell_orders.get(ba, 0)
        if bv1 < 0: bv1 = 0
        if av1 < 0: av1 = 0
        ...
        # ── BUY: spread stretto + momentum bullish ─────────────
        # EV +1.38 tick solo con spread≤6 (spread 7-9 → EV negativo)
        if spread <= self.NARROW_BUY_SPREAD_MAX and imbtot > 0 and buy_cap > 0:
            avail_at_ask = -od.sell_orders.get(ba, 0)
            qty = min(avail_at_ask, buy_cap, self.NARROW_TAKE_SIZE)
            if qty > 0:
                orders.append(Order(product, ba, qty))
                buy_cap -= qty

        # ── SELL: spread stretto + momentum bearish ────────────
        # EV +0.42 tick con spread≤7 (spread 8-9 → EV negativo)
        elif spread <= self.NARROW_SELL_SPREAD_MAX and imbtot < 0 and sell_cap > 0:
            avail_at_bid = od.buy_orders.get(bb, 0)
    ...
    def _trade_tomatoes(self, state: TradingState, result: Dict[str, List[Order]]) -> None:
        ...
        self._log_narrow_regime_snapshot(state, product, order_depth=state.order_depths[product])
